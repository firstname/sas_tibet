...
%do i=1 %to &num_of_zsd.+0;
        ...
        proc means data=&score. noprint ;
                class &score.;
                var  zsd_&i.;
                output out=jiben  mean=pj  ;
                label &score. = "&coursename.得分"
                      pj = "&name_of_zsd.平均得分"   ;
                format   &score. five.;
                run;
        proc print data=jiben nobs label;
                var  &score. pj;
                run;
        proc gplot data=jiben;
                plot  pj * &score./ ctext= blue;
                symbol   i=spline  v=point cv=red ci=black;
                label &score. = "&coursename.得分"
                      pj = "&name_of_zsd.平均分"   ;
                run;quit;
        %end;
